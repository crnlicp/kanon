import { aA as UserRole, aB as SubmissionStatus, aC as Topic, aD as BackgroundScope } from "./index-BsZ-DJdz.js";
const mockBlob = (url) => ({
...
const mockBackend = {
  ...
  addActivity: async (_token, input) => ok({ ...input, id: BigInt(Date.now()) }),
  addFooterLink: async (_token, input) => ok({ ...input, id: BigInt(Date.now()) }),
  addHeroSlide: async (_token, input) => ok({ ...input, id: BigInt(Date.now()) }),
  addArea: async (_token, input) => {
    const newArea = {
      id: BigInt(Date.now()),
      icon: input.icon,
  ...
  setAreaCardBackground: async (_token, areaId, blob) => {
    const area = mockAreas.find((a) => a.id === areaId);
    if (!area) return ok(null);
    area.cardBackground = blob;
    return ok(area);
  },
  setAreaBackground: async (_token, areaId, blob) => {
    const area = mockAreas.find((a) => a.id === areaId);
    if (!area) return ok(null);
    area.areaBackground = blob;
    return ok(area);
  },
  setAreaBackgroundVideo: async (_token, areaId, blob) => {
    const area = mockAreas.find((a) => a.id === areaId);
    if (!area) return ok(null);
    area.areaBackgroundVideo = blob;
    return ok(area);
  },
  deleteActivity: async (_token) => ok(true),
  deleteFooterLink: async (_token) => ok(true),
  deleteHeroSlide: async (_token) => ok(true),
  deleteSubmission: async (_token) => ok(true),
  deleteContactSubmission: async (_token, _id) => ok(true),
  updateActivity: async (_token) => ok(true),
  updateFooterLink: async (_token) => ok(true),
  updateHeroSlide: async (_token) => ok(true),
  updateSubmissionStatus: async (_token) => ok(true),
  reorderFooterLinks: async (_token) => ok(true),
  reorderHeroSlides: async (_token) => ok(true),
  setBackground: async (_token, input) => ok(input),
  setContactInfo: async (_token) => ok(true),
  setSiteSettings: async (_token) => ok(true),
  updateAdminPassword: async (_token, _newPassword) => {
    sampleSettings.adminPassword = _newPassword;
    return ok(true);
  },
  emergencyResetPassword: async (_recoverySecret) => {
    return true;
  },
  resetAdminPassword: async (_token) => ok(true),
  setAbout: async (_token, _input) => true,
  getContactSubmissions: async (_token) => ok([
    {
      id: BigInt(1),
      name: "Sara Lindqvist",
  ...
  assignCallerUserRole: async (_user, _role) => void 0,
  ...
  _immutableObjectStorageConfirmBlobDeletion: async () => void 0,
  _immutableObjectStorageCreateCertificate: async (_blobHash) => ({ method: "GET", blob_hash: "" }),
  _immutableObjectStorageRefillCashier: async (_info) => ({ success: true }),
  _immutableObjectStorageUpdateGatewayPrincipals: async () => void 0
};
export {
  mockBackend
};
